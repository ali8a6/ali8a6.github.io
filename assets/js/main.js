...
(function () {
  ...
  if (heroImg) {
    ...
    const defaultCandidates = ['.jpg', '.jpeg', '.png', '.webp', '.avif'];
    // prefer the extension already present in the HTML src (avoid unnecessary 404s)
    const currentSrc = heroImg.getAttribute('src') || '';
    const extMatch = currentSrc.match(/\.(jpg|jpeg|png|webp|avif)(?:$|\?)/i);
    const candidates = extMatch ? [
      '.' + extMatch[1].toLowerCase(),
      ...defaultCandidates.filter(e => e !== '.' + extMatch[1].toLowerCase())
    ] : [...defaultCandidates];
    ...
    // Start with the preferred candidate (current extension if present)
